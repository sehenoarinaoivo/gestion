...
if(!empty($_POST['lieu']) and !empty($_POST['date_debut']) and !empty($_POST['date_fin'])){
	...
	$compte=mysqli_fetch_array(mysqli_query("select count(*) as nb from stage where lieu_stage='$lieu' and numel='$numel' and date_debut='$date_debut' and date_fin='$date_fin'"));
	$bool=true;
	if($compte['nb']>0){
	$bool=false;
...
$donnee=mysqli_fetch_array(mysqli_query("select * from stage where numstage='$id'")); //	On selectione les champ qu'on va modifier dans la table stage
$lieu=$donnee['lieu_stage'];
$date_debut=$donnee['date_debut'];
$date_fin=$donnee['date_fin'];
$data=mysqli_fetch_array(mysqli_query("select numel,nomel,prenomel from eleve where numel=(select numel from stage where numstage='$id')"));// 	on reselectionne le numel pour que �a soit similaire � la requete de l'ajout
}
